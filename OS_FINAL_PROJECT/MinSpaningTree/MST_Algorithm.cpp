...
Graph* Prim::operator()(Graph *g) {
    ...
    Graph *mst = new Graph(*g, false);  // Create a new graph for the Minimum Spanning Tree (MST) with the same vertices but no edges
    const int INTINF = std::numeric_limits<int>::max(); // Define infinity value for initialization
    BinaryHeap<std::pair<size_t, int>, CompareVertex> minHeap;// Fibonacci min Heap to select the next vertex with the minimum key value
    std::vector<int> key(V, INTINF); // Key values (weights) used to pick the minimum weight edge for each vertex
    std::vector<int> parent(V, -1);  // Array to store the parent of each vertex in the MST
    std::vector<bool> inMST(V, false);// Boolean array to track vertices already included in the MST
