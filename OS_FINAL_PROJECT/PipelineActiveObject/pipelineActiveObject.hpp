...
private:
    ...
    struct Worker {
        ...
        std::queue<void*> task_q;  // Queue of tasks assigned to the worker (tasks are of generic type `void*`)
        ...
        std::queue<void*>* next_task_q;  // Pointer to the task queue of the next worker in the pipeline
    };

    // Function that defines the behavior of a worker thread. Processes tasks and passes them to the next worker.
    void workerFunction(Worker& worker, Worker* next_worker);

    std::vector<Worker> workers;  // Vector holding all worker threads
    std::atomic<bool> f_stop;   // Atomic flag used to signal workers to stop
};
