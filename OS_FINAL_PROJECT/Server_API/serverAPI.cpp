...
std::pair<std::string, Graph *> handle_input(Graph *g, std::string act, int fd_client, std::string current_act, int n, int m, int w, std::string algo){
    std::string msg;
...
void delete_from_pfds(struct pollfd pfds[], int i, int *count){
...
std::pair<std::string, Graph *> newGraph(int n, int m, int fd_client, Graph *g){
    ...
    std::unordered_set<Vertex> vertices = init_vertx(n); // Initialize the vertices
    g = new Graph(vertices);   // Create a new graph of n vertices
    init_graph(g, m, fd_client); // Initialize the graph with m edges
...
void parse_input(char *buf, int numOfBytes, int &n, int &m, int &weight, std::string &strat, std::string &act, std::string &current_act, const std::vector<std::string> &commands_graph, const std::vector<std::string> &mst_starts){
