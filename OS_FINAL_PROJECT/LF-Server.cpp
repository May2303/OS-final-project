            ...
            if (pfds[i].revents & POLLIN) { // Data is ready to read
                if (pfds[i].fd == listener) {
                    ...
                    if (num_of_bytes <= 0) {
                        ...
                        delete_from_pfds(pfds, i, &fd_count); // Remove the file descriptor from the pollfd array
                        ...
                        parse_input(buf, num_of_bytes, n, m, weight, strat, action, current_act, commands_graph, mstStrats);
                        cout << "Act received: " << action << " from client: " << sender_fd << endl;

                        // Handle input and perform appropriate actions
                        pair<string, Graph *> result = handle_input(clients_graphs[sender_fd], action, sender_fd, current_act, n, m, weight, strat);
